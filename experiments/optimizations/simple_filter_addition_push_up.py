...
def execute_filter_addition_push_up_average_case(scale_factor, tmpdir, variant_count):
    ...
    dag_extraction_result = PipelineAnalyzer \
        .on_pipeline_from_string(test_code) \
        .overwrite_optimization_rules([SimpleFilterAdditionPushUp(singleton, disable_selectivity_safety=True)]) \
        .execute() \
        .dag_extraction_info
    analysis_result_with_opt_rule = PipelineAnalyzer \
        .on_previously_extracted_pipeline(dag_extraction_result) \
...
def execute_filter_addition_push_up_worst_case_no_original_pipeline_heuristic(scale_factor, tmpdir, variant_count):
    data_size = int(950 * scale_factor)
    df_a_train, df_b_train = get_test_df(int(data_size * 0.8))
    df_a_path_train = os.path.join(tmpdir, "filter_push_up_df_a_worst_case_no_original_train.csv")
    df_a_train.to_csv(df_a_path_train, index=False)
    ...
    for variant_index in range(variant_count):
        ...
        filter_variants[f"filter_{variant_index}"] = ("B", partial(filter_func, bound_variant_index=variant_index))
    estimated_selectivities = [0.05 for _ in range(variant_count)]
    data_corruption = DataFilterVariants(filter_variants, False, estimated_selectivities=estimated_selectivities)
    dag_extraction_result = PipelineAnalyzer \
        .on_pipeline_from_string(test_code) \
        .execute() \
        .dag_extraction_info
...
scenario_funcs = {
    ...
    'worst_w_original_w_heuristic': execute_filter_addition_push_up_worst_case_no_original_pipeline_heuristic
}
